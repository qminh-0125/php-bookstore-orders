...
<div class="form-container">
    <h2>Chỉnh sửa Đơn hàng Sách</h2>
    <p class="form-subtitle">Cập nhật chi tiết đơn hàng (ID: #<?= e($id) ?>).</p>

    <form method="post" action="/book-orders/update" class="form-card">
        <!-- Hidden ID -->
        ...
        <div class="form-group">
            ...
            <select id="status" name="status" class="<?= isset($errors['status']) ? 'input-error' : '' ?>">
                <option value="pending" <?= ($old['status'] ?? '') === 'pending' ? 'selected' : '' ?>>Chờ thanh toán (Pending)</option>
...
$title = 'Sửa Đơn hàng - Bookstore App';
